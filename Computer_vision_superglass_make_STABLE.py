        ...
        while bank_dump_confidence[1] < BANK_DUMP_THRESHOLD:
            ...
            if search_time > 5:
                ...
                if bank_confidence[1] < BANK_THRESHOLD_RECLICK:
                    print('5+ seconds inactivity. reclick failed due to low confidence. bank confidence %s | BANK_THRESHOLD DISABLED(!) %s | Exiting...' %( bank_confidence[1], BANK_THRESHOLD))
                    exit()
                else:
                    print('5+ seoncds inactivity. attempting reclick on bank and sleeping 5s. bank confidence %s | BANK_THRESHOLD DISABLED(!) %s | Continuing...' %( bank_confidence[1], BANK_THRESHOLD))
                    bank_screenPoint = wincap.get_screen_position(bank_confidence[0])
                    bank_clickPoint = bank_action.click(bank_screenPoint, speed = SPEED)
