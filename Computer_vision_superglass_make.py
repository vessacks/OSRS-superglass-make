...
#make sure the bank image works. the current bank image is in lovakenj bank, north west booth, looking due north
...
def superglass_make():    
    while True:
        ...
        sleepytime = 1.3 + abs(np.random.normal(0,.1))
        time.sleep(sleepytime)        


        #start clicking bank hoping to get in. stop when you see the bank dump button
        bank_dump_confidence = [0,0]
        ...
        while bank_dump_confidence[1] < BANK_DUMP_THRESHOLD:
            ...
            #if you see the bank dump stop clicking
            if bank_dump_confidence[1] > BANK_DUMP_THRESHOLD: 
                print('inside the bank, bank_dump_confidence = %s' %bank_dump_confidence)
                break
            if cv.waitKey(1) == ord('q'):
                    cv.destroyAllWindows()
                    exit()            
            
            #if you get here, you haven't enteredthe bank. click bank to enter
            screenshot = wincap.get_screenshot()
            screenshot = cv.cvtColor(screenshot, cv.COLOR_BGR2GRAY)
            ...
            if search_time > 2:
                print('bank clicked many times, not working. exiting')
                exit()

            #debug
            sleepytime = .10 + abs(np.random.normal(.05,.15))
            print('clicked, waiting %ss | bank confidence %s | BANK_THRESHOLD %s | bank_dump_confidence %s | BANK_DUMP_THRESHOLD %s ' %( sleepytime, bank_confidence[1], BANK_THRESHOLD, bank_dump_confidence[1], BANK_DUMP_THRESHOLD))

            #wait a bit before reclicking
            time.sleep(sleepytime)
        ...
        if bank_confidence[1] < BANK_THRESHOLD:
            print('bank confidence %s | BANK_THRESHOLD %s | Exiting...' %( bank_confidence[1], BANK_THRESHOLD))
            exit()
        else:
        ...
        while bank_dump_confidence[1] < BANK_DUMP_THRESHOLD:
            ...
            if search_time > 5:
                ...
                if bank_confidence[1] < BANK_THRESHOLD_RECLICK:
                    print('5+ seconds inactivity. reclick failed due to low confidence. bank confidence %s | BANK_THRESHOLD %s | Exiting...' %( bank_confidence[1], BANK_THRESHOLD))
                    exit()
                else:
                    print('5+ seoncds inactivity. attempting reclick on bank. bank confidence %s | BANK_THRESHOLD %s | Continuing...' %( bank_confidence[1], BANK_THRESHOLD))
                bank_screenPoint = wincap.get_screen_position(bank_confidence[0])
                bank_clickPoint = bank_action.click(bank_screenPoint, speed = SPEED)
                ...
                if cv.waitKey(1) == ord('q'):
                        ...
                        exit()
            if search_time > 20:
                print('20s+ inactivity. multiple reclicks failed. giving up, exitting...')
                exit()
        '''
        ...
        if spellcast_confidence[1] < SPELLCAST_THRESHOLD:
            print('spellcast confidence %s | SPELLCAST_THRESHOLD %s | breaking loop and retrying...' %( spellcast_confidence[1], SPELLCAST_THRESHOLD))
            break
        else:
