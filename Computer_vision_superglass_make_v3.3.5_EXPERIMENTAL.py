...
def wait():
    wait = (.03 + abs(np.random.uniform(0,.02)))
    return wait

def speed():
    speed = np.random.normal(.78,.3)
    while speed > .9 or speed < .6:
        ...
        while bank_dump_confidence[1] < BANK_DUMP_THRESHOLD:
            ...
            if search_time > 2:
                ...
                if bank_confidence[1] < BANK_THRESHOLD_RECLICK:
                    ...
                    print('2+ seoncds inactivity. attempting reclick on bank and sleeping ~2s. bank confidence %s | BANK_THRESHOLD DISABLED(!) %s | Continuing...' %( bank_confidence[1], BANK_THRESHOLD))
                bank_screenPoint = wincap.get_screen_position(bank_confidence[0])
                bank_clickPoint = bank_action.click(bank_screenPoint, speed = speed(),wait = wait())
                #wait to get to bank
                time.sleep(1 + abs(np.random.normal(0,1)))
